...
describe("8 Promises", () => {
  ...
  describe("async await", () => {
    ...
    it("can find Ham and then its details", async () => {
      const product = await api.fetchProduct("ham");
      expect(product).toEqual([{ id: 5, name: "ham", price: 1 }]);
      const hamDetails = await api.fetchProductDetails(product[0].id);
      expect(hamDetails).toEqual({
        description: "is great smoked!",
        id: 5,
        name: "ham"
      });
    });
